...
class Modelgenerator:
    def __init__(self):
        ...
        self.pipe = ShapEPipeline.from_pretrained("openai/shap-e", torch_dtype=dtype)
    ...
    def generate(self, prompt):
        ...
        ply_name = f"{safe_name}.ply"
        glb_name = f"{safe_name}.glb"

        model = self.pipe(prompt, guidance_scale=15.0, num_inference_steps=70, output_type="mesh")

        export_to_ply(model.images[0], ply_name)

        mesh = trimesh.load(ply_name)
        mesh.export(glb_name)
